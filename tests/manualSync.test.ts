...
const config = {
  ...
  profileId: 'default',
  objectPrefix: 'Forge-OS_Base/Domain1127',
  credentialProviderUrl: 'https://example.com/cos-credentials',
};

const storageRecord = {
  'alo-storage': JSON.stringify({
    state: {
      tasks: [],
      calendarEvents: [],
      principles: [],
...
test('runManualSync writes a history snapshot when backup key is provided', async () => {
  ...
  const result = await runManualSync({
    ...
    client,
    key: 'Forge-OS_Base/Domain1127/alo-data.sync.json',
    backupKey: 'Forge-OS_Base/Domain1127/snapshots/2026-06-03T00-00-00.000Z-device-1.json',
    appVersion: '1.0.2',
    deviceId: 'device-1',
    storageRecord,
    now: '2026-06-03T00:00:00.000Z',
  ...
  assert.deepEqual(uploaded.map((item) => [item.key, item.backup]), [
    ['Forge-OS_Base/Domain1127/alo-data.sync.json', false],
    ['Forge-OS_Base/Domain1127/snapshots/2026-06-03T00-00-00.000Z-device-1.json', true],
  ]);
});
...
test('runManualSync marks conflict and backs up local data before overwrite', async () => {
  ...
  const result = await runManualSync({
    ...
    client,
    key: 'Forge-OS_Base/Domain1127/alo-data.sync.json',
    backupKey: 'Forge-OS_Base/Domain1127/snapshots/local-conflict.json',
    appVersion: '1.0.2',
    deviceId: 'device-1',
    storageRecord,
    lastSyncedRevision: 'remote-1',
...
test('runManualSync does not conflict with a newer remote revision from the same device', async () => {
  ...
  const result = await runManualSync({
    ...
    client,
    key: 'Forge-OS_Base/Domain1127/alo-data.sync.json',
    backupKey: 'Forge-OS_Base/Domain1127/snapshots/2026-06-03T09-21-46.035Z-device-1.json',
    appVersion: '1.0.2',
    deviceId: 'device-1',
    storageRecord,
  ...
  assert.deepEqual(uploaded.map((item) => [item.key, item.backup]), [
    ['Forge-OS_Base/Domain1127/alo-data.sync.json', false],
    ['Forge-OS_Base/Domain1127/snapshots/2026-06-03T09-21-46.035Z-device-1.json', true],
  ]);
});
