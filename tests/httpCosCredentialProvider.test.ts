...
test('HTTP credential provider posts object request and returns a signed URL', async () => {
  ...
  const provider = createHttpCosCredentialProvider({
    ...
    fetcher: async (input, init) => {
      ...
      return new Response(
        JSON.stringify({
          url: 'https://signed.example.com/alo-data.sync.json',
  ...
  assert.equal(JSON.parse(calls[0].init?.body as string).key, 'Forge-OS_Base/Domain1127/alo-data.sync.json');
  assert.equal(signed.url, 'https://signed.example.com/alo-data.sync.json');
