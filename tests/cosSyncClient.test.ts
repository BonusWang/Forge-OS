...
test('cos sync client downloads a snapshot and reads its revision', async () => {
  ...
  const revision = await client.readRemoteRevision('Forge-OS_Base/Domain1127/alo-data.sync.json');
  const downloaded = await client.downloadSnapshot('Forge-OS_Base/Domain1127/alo-data.sync.json');
...
test('cos sync client uploads primary and backup snapshots through signed PUT urls', async () => {
  ...
  await client.uploadSnapshot('Forge-OS_Base/Domain1127/alo-data.sync.json', envelope);
  await client.uploadBackupSnapshot('Forge-OS_Base/Domain1127/snapshots/revision-1.json', envelope);
...
test('cos sync client reports fetch failures with a COS connectivity hint', async () => {
  ...
  await assert.rejects(
    () => client.downloadSnapshot('Forge-OS_Base/Domain1127/alo-data.sync.json'),
    /浏览器无法访问 COS/
  );
});
