...
test('runCloudBackup uploads both the primary cloud object and a history snapshot', async () => {
  ...
  assert.equal(result.backupKey, 'Forge-OS_Base/Domain1127/snapshots/2026-06-03T00-00-00.000Z-device-1.json');
...
test('restoreCloudSnapshot restores a selected cloud history snapshot', async () => {
  ...
  const result = await restoreCloudSnapshot({
    config,
    client,
    key: 'Forge-OS_Base/Domain1127/snapshots/2026-06-03T00-00-00.000Z-device-2.json',
    appVersion: '1.0.2',
  });
