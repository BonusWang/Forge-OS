...
test('V3 runner rejects remote namespace mismatch without mutating local state', async () => {
  const remote = await createV3SyncEnvelope({
    ...
    deviceId: 'other',
    owner: {
      namespace: 'Forge-OS_Base/Domain1127/GoogleChrome/profiles/work',
      profileId: 'work',
    },
...
test('V3 runner refuses clean initialization when legacy main objects still exist', async () => {
  ...
  const client: V3SyncClient = {
    ...
    objectExists: async (key) => key.endsWith('alo-data.sync.json'),
  };
  ...
  const result = await runV3Sync({
    ...
    key: v3SyncObjectKey(config),
    legacyObjectKeys: [
      'Forge-OS_Base/Domain1127/GoogleChrome/alo-data.sync.json',
      'Forge-OS_Base/Domain1127/GoogleChrome/alo-data.entities.v2.json',
    ],
    appVersion: '1.0.5',
    deviceId: 'browser',
    state: baseState(),
  ...
  assert.equal(result.phase, 'error');
  if (result.phase === 'error') {
    assert.equal(result.error, 'legacy-main-object-present');
  }
  assert.equal(uploads.length, 0);
});
