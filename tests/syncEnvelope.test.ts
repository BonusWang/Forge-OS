...
test('createSyncEnvelope preserves alo-storage and validates checksum', async () => {
  ...
  assert.equal(envelope.storageRecord['alo-storage'], storageRecord['alo-storage']);
...
test('validateSyncEnvelope rejects snapshots without alo-storage', async () => {
  const envelope = await createSyncEnvelope({
    appVersion: '1.0.2',
    deviceId: 'device-1',
    revision: 'revision-1',
