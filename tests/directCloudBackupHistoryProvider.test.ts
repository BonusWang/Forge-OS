...
test('direct cloud backup history provider lists signed COS snapshots', async () => {
  ...
  const provider = createDirectCloudBackupHistoryProvider({
    ...
    fetcher: async (input) => {
      ...
      return new Response(
        ...
        <ListBucketResult>
          <Contents>
            <Key>Forge-OS_Base/Domain1127/snapshots/2026-06-02T00-00-00.000Z-device-1.json</Key>
            <LastModified>2026-06-02T00:00:00.000Z</LastModified>
            <Size>128</Size>
          </Contents>
          <Contents>
            <Key>Forge-OS_Base/Domain1127/snapshots/2026-06-03T00-00-00.000Z-device-1.json</Key>
  ...
  assert.match(requested[0], /prefix=Forge-OS_Base%2FDomain1127%2Fsnapshots%2F/);
  assert.equal(snapshots[0].key, 'Forge-OS_Base/Domain1127/snapshots/2026-06-03T00-00-00.000Z-device-1.json');
