...
test('HTTP cloud backup history provider lists snapshots next to the signer endpoint', async () => {
    ...
    fetcher: async (input, init) => {
      ...
      return new Response(
        JSON.stringify({
          snapshots: [
            {
              key: 'Forge-OS_Base/Domain1127/snapshots/2026-06-03T00-00-00.000Z-device-1.json',
  ...
  assert.equal(JSON.parse(calls[0].init?.body as string).objectPrefix, 'Forge-OS_Base/Domain1127');
