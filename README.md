...
- Claude 视觉风格替换旧暖色参考风格，桌面端和手机端共享同一视觉 token，并兼容旧 `orbit` 偏好。
- 手机端「今日」收敛为轻量「今日焦点」状态卡，新增任务入口移动到「今日承诺」标题右侧，减少右下角 FAB 对内容的遮挡。
- 手机端「记录」改为「灵感 / 反思」分段切换，两者共用纵向流程链，但保持步骤文案、保存路径和最近记录独立。
- Android 构建在同步 Web 资源前会清理旧生成目录，避免旧 Vite 产物残留进 APK。
- 新增产品设计专家 skill，并沉淀 Forge 与 matrix-life-os 的专家团评审报告和桌面端/移动端优化样例图。
- OpenSpec 补充 Claude 视觉系统调整、移动端记录与今日页体验优化要求。
