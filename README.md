...
- COS 日常跨端同步升级为 V3 实体级同步，引入 `alo-data.entities.v3.json`、命名空间/profile 隔离和本地 V3 基线。
- V3 同步支持三方合并、删除 tombstone、字段级冲突和冲突去重，减少浏览器与 Android 并发修改时的手工恢复。
- 系统同步面板展示 V3 namespace、revision、初始化状态、自动合并数量、冲突数量和最近错误。
- 手机端「今日」改为轻量「今日焦点」加「今日承诺」行动台，新增任务入口收敛到今日承诺标题右侧。
- 手机端「记录」改为「灵感 / 反思」分段切换，灵感和反思共享流程框架但保持字段、文案和保存路径独立。
- 新增 Dossier 视觉风格，并支持手机端使用独立于桌面端的视觉风格偏好。
- OpenSpec 已归档 V3 同步引擎规格，并补充移动端记录页与今日页体验要求。
