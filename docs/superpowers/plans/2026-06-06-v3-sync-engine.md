...
const baseConfig = {
  objectPrefix: 'Forge-OS_Base/Domain1127/GoogleChrome',
  profileId: 'default',
};
...
test('V3 sync key is isolated by profile namespace', () => {
  assert.equal(
    ...
    'Forge-OS_Base/Domain1127/GoogleChrome/profiles/default/alo-data.entities.v3.json'
  );
  assert.equal(
    v3SyncObjectKey({ ...baseConfig, profileId: 'work' }),
    'Forge-OS_Base/Domain1127/GoogleChrome/profiles/work/alo-data.entities.v3.json'
  );
});
...
test('V3 namespace can reserve a future user id', () => {
  assert.equal(
    ...
    'Forge-OS_Base/Domain1127/GoogleChrome/users/user-a/profiles/default'
  );
});
```
...
Run a read-only COS listing for `Forge-OS_Base/Domain1127/GoogleChrome/` and confirm no old `alo-data.sync.json`, `alo-data.entities.v2.json`, or `alo-data.entities.v3.json` exists before the browser initializes V3.
