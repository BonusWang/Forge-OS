...
const hasLegacyMainObject = async (
  client: V3SyncClient,
  legacyObjectKeys: string[] | undefined
): Promise<boolean> => {
  if (!client.objectExists || !legacyObjectKeys?.length) return false;

  for (const key of legacyObjectKeys) {
    if (await client.objectExists(key)) {
      return true;
    }
  }
...
export const runV3Sync = async ({
  ...
  try {
    ...
    if (!remoteEnvelope) {
      if (await hasLegacyMainObject(client, legacyObjectKeys)) {
        return { phase: 'error', error: 'legacy-main-object-present' };
      }
