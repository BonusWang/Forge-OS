...
export const validateSyncEnvelope = async (
  ...
  if (typeof envelope.storageRecord['alo-storage'] !== 'string') {
    return { ok: false, error: 'missing-alo-storage' };
  }
...
export const restoreAppStateFromSyncEnvelope = async (
  ...
  try {
    parsedStorage = JSON.parse(validation.envelope.storageRecord['alo-storage']);
  } catch {
    return { ok: false, error: 'invalid-alo-storage' };
  }
